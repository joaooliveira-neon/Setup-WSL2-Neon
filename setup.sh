...
# Some tools
sudo apt-get install -y -qq build-essential git zsh neovim gedit fzf tmux eza bat colordiff ripgrep pipx unzip \
fd-find libssl-dev zlib1g-dev libbz2-dev libreadline-dev libsqlite3-dev wget curl llvm gettext telnet xdg-utils \
libncurses5-dev tk-dev tcl-dev blt-dev libgdbm-dev git aria2 lzma liblzma-dev python3 python3-pip net-tools 2>&1 | while read -r line; do
    echo -ne "\r\033[K${line}"
done
echo
# Zoxide
